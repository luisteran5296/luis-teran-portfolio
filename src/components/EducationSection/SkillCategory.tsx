...
export default function ProfessionalProfile() {
  ...
  const technicalSkills = [
    { name: "Machine Learning & Deep Learning", level: 95, icon: Brain, color: "text-purple-400" },
    { name: "Python & Data Engineering", level: 95, icon: Code2, color: "text-blue-400" },
    { name: "Google Cloud Platform & Vertex AI", level: 93, icon: Cloud, color: "text-cyan-400" },
    { name: "SQL, BigQuery & Stored Procedures", level: 92, icon: Database, color: "text-amber-400" },
    { name: "AWS & Serverless Architecture", level: 88, icon: Server, color: "text-emerald-400" },
    { name: "Docker & Container Services", level: 87, icon: Container, color: "text-sky-400" },
  ];

  const softSkills = [
    { name: "Agentes Autónomos & LLMs", icon: Crown, color: "text-amber-400 border-amber-500/30 bg-amber-500/10" },
    { name: "Visión por Computadora", icon: Sparkles, color: "text-purple-400 border-purple-500/30 bg-purple-500/10" },
    { name: "NLP & Text Mining", icon: Workflow, color: "text-sky-400 border-sky-500/30 bg-sky-500/10" },
    { name: "Pensamiento de Producto", icon: Lightbulb, color: "text-yellow-400 border-yellow-500/30 bg-yellow-500/10" },
    { name: "Arquitecturas Serverless", icon: Cloud, color: "text-emerald-400 border-emerald-500/30 bg-emerald-500/10" },
    { name: "Modelado Predictivo", icon: Brain, color: "text-rose-400 border-rose-500/30 bg-rose-500/10" },
  ];
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-2">
        ...
        <h3 className="text-2xl md:text-3xl font-extrabold tracking-tight">{t.education.arsenalTitle}</h3>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-[2rem] border border-foreground/15 shadow-xl relative overflow-hidden">
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-border/60">
            <h4 className="text-lg sm:text-xl font-bold text-foreground flex items-center gap-2">
              <Server className="w-5 h-5 text-primary" /> {t.education.arsenalTitle}
            </h4>
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-[2rem] border border-foreground/15 shadow-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-8 pb-4 border-b border-border/60">
              <h4 className="text-lg sm:text-xl font-bold text-foreground flex items-center gap-2">
                <Brain className="w-5 h-5 text-primary" /> {t.education.traitsTitle}
              </h4>
              <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground bg-muted/60 px-3 py-1 rounded-full border border-border/50">
                Core
              </span>
                ...
                {softSkills.map((skill, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      className={`px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-2xl border text-xs sm:text-sm font-semibold flex items-center gap-2 shadow-sm hover:scale-105 transition-transform cursor-default ${skill.color}`}
                    >
            ...
            <div className="p-4 rounded-2xl bg-gradient-to-r from-primary/10 via-purple-500/5 to-transparent border border-primary/20 flex items-start gap-3.5 shadow-sm">
              ...
              <div>
                <strong className="text-foreground font-bold text-sm block mb-0.5">
                  {t.education.footerNoteTitle}
                </strong>
                <p className="text-xs text-muted-foreground leading-relaxed">
                  {t.education.footerNoteDesc}
                </p>
              </div>
            </div>
          </div>
